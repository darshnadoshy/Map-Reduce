...
void MR_Emit(char * key, char * value) {
  ...
  table[pno][pnum[pno].index].key = (char * )
  malloc(sizeof(char) * strlen(key));
  if (table[pno][pnum[pno].index].key == NULL) {
    printf("Memory Allocation Failed!\n");
    exit(1);
  }
  table[pno][pnum[pno].index].value = (char * )
...
Reducer reduce, int num_reducers, Partitioner partition, int num_partitions) {
  ...
  pthread_t *p = (pthread_t *)malloc(sizeof(pthread_t)*num_mappers);
  if (p == NULL) {
    printf("Memory Allocation Failed\n");
    exit(1);
  }
  ...
  for (i = 0; i < mthread_size; i++) {
    fname[i].index = 0;
    fname[i].name = (char ** ) malloc(sizeof(char * ) * (argc - 1));
  }
  ...
  for (i = 1; i < argc; i++) {
    pos = i % num_mappers;
    if (pos == 0) {
      fname[num_mappers - 1].name[fname[num_mappers - 1].index] = (char * )
      malloc(sizeof(char) * strlen(argv[i]));
      strcpy(fname[num_mappers - 1].name[fname[num_mappers - 1].index],
      argv[i]);
      fname[num_mappers - 1].index++;
    } else {
      fname[pos - 1].name[fname[pos - 1].index] = (char * )
      malloc(sizeof(char) * strlen(argv[i]));
  ...
  for (i = 0; i < mthread_size; i++) {
    // Create threads only if there are files assigned to it.
    if (fname[i].index != 0)
  ...
  for (i = 0; i < rthread_size; i++) {
    part[i].index = 0;
    part[i].partition_no = (unsigned long * )
    malloc(sizeof(unsigned long) * num_partitions);
  }
